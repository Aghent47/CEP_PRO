...
const AttributeControlChart: React.FC<AttributeControlChartProps> = ({
  title,
  chartType,
  values,
  centerLine,
  ...
  // Obtener nombre del eje Y según tipo de gráfico
  const getYAxisName = (): string => {
    switch (chartType) {
      case 'p': return 'Proporción de defectuosos (p)';
      case 'np': return 'Número de defectuosos (np)';
      case 'c': return 'Número de defectos por unidad (c)';
      case 'u': return 'Defectos por unidad (u)';
  ...
  useEffect(() => {
    ...
    const minUcl = Math.min(...ucl);
    const maxLcl = Math.max(...lcl);
    const yMin = Math.min(minDataValue, maxLcl) - 0.05;
    const yMax = Math.max(maxDataValue, minUcl) + 0.05;
    ...
    const option: echarts.EChartsOption = {
      ...
      tooltip: {
        ...
        formatter: (params: any) => {
          ...
          let sampleInfo = '';
          if (sampleSizes[dataPoint.dataIndex]) {
            sampleInfo = `<br/>Tamaño de muestra: ${sampleSizes[dataPoint.dataIndex]}`;
          }
          
          return `
            <strong>Subgrupo ${dataPoint.dataIndex + 1}</strong><br/>
            Valor: ${dataPoint.value.toFixed(4)} ${unit}${sampleInfo}<br/>
            LCS: ${ucl[dataPoint.dataIndex].toFixed(4)} ${unit}<br/>
            LC: ${centerLine.toFixed(4)} ${unit}<br/>
            LCI: ${lcl[dataPoint.dataIndex].toFixed(4)} ${unit}
          `;
        }
      },
      grid: {
          ...
          tooltip: {
            formatter: (params: any) => {
              return `
                ...
                LCS: ${ucl[params.dataIndex].toFixed(4)} ${unit}<br/>
                LCI: ${lcl[params.dataIndex].toFixed(4)} ${unit}
              `;
            }
          }
        },
      ...
      toolbox: {
        feature: {
          saveAsImage: { 
            ...
            name: `grafico_atributos_${title.replace(/\s/g, '_')}`
          },
  ...
  }, [values, centerLine, ucl, lcl, outOfControlPoints, title, chartType, sampleSizes, unit]);

  const getChartTitle = (): string => {
    const chartNames = {
      p: 'Gráfico p (Proporción de unidades defectuosas)',
      np: 'Gráfico np (Número de unidades defectuosas)',
      c: 'Gráfico c (Número de defectos por unidad)',
      u: 'Gráfico u (Defectos por unidad)'
    };
    return chartNames[chartType];
  };
